...
| gap | 2 | 24–25 | |
| value | **52** | 26–77 | longest drawn is `On, overridden by ZERADO_NO_AUDIO` = 33 |
| | **74** | | |
...
**22 content rows in a 15-row scroll region.** Row 16 of the body is the position line, pinned
...
0.........1.........2.........3.........4.........5.........6.........7.........
   ...
   ↑↓ move   ⏎ replace   esc back   ? help   q quit
...
(2.4.11) — the `STEAM` head above it and a settings row below it both stay visible.
...
**All 22 rows fit with room to spare — no scrolling at all.** That is the whole progressive
enhancement of the extra height, and it is why the position line disappears when there is nothing
to position.
...
is satisfied at every tier and the extra width is spent on the thing that actually helps: **all 22
...
countable: this screen has **four** uppercase group heads and **eighteen** labels. Uppercasing all
twenty-two would leave nothing to outrank anything, and the group heads — which are the screen's
...
| position line | `--z-text-secondary` | `#A9B5C7` | **249** | `white` | **9.36** | `▄` U+2584 — **Ambiguous**, flowing, must be width-measured | `1–15 of 22` | **yes** |
...
Four rows on this screen are pure readouts (`Account`, `Output`'s failure forms, `Colour`, `File`,
`Size`). They must be distinguishable from actionable rows **without colour**, and the product has
no spare glyph.
...
| Focused row | Footer |
|---|---|
| `Account`, `File`, `Size`, `Colour`, `Output` (when it is a failure sentence) | `↑↓ move   esc back   ? help   q quit` |
...
A value too long for 34 columns **wraps**; it is never truncated. A setting whose value is cut off
is a setting the player cannot verify, which is this screen's one failure mode.

**Two-line rows grow the list from 22 lines to 36**, which the position line states honestly. And
...
Responsive table: *"Same"* as Wide. Label 18 + gap 2 + value **32**. The longest value —
`On, overridden by ZERADO_NO_AUDIO`, 33 — is **one column over**, so at Standard it wraps to a
second line; every other value fits. Stated as arithmetic rather than as a judgement.
...
At Tiny the list is **37 lines** in a 20-line region and the position line says so. **Nothing is
...
| that a setting is overridden by the environment | the value **names the variable** — `NO_COLOR`, `ZERADO_ASCII`, `ZERADO_NO_AUDIO` |
| that there is more below | `▄ 1–15 of 22` |
...
1. **No value is behind a submenu.** Every value in §10 is readable by scrolling and pressing
   nothing. Grep the render at `120 × 40`: all 22 rows present, no scrolling required.
...
14. Eight artifacts per [`03-responsive.md`](../../blueprint/03-responsive.md) §7, **plus**
    `80 × 24` for: not-connected, `file` backing, each audio-unavailable reason, and the
    confirmation.
