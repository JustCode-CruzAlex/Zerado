...
what the player sees**) · [`../../blueprint/16-i18n.md`](../../blueprint/16-i18n.md) (§3.1's
...
| 8 | **The design brief's accessibility table says Zerado has no images.** [`../00-design-brief.md`](../00-design-brief.md) §3 reads *"**1.4.5 / 1.4.9** Images of Text — No images. Cover art arrives in Phase 2 and is art, not text."* The **conclusion** survives — a cover is artwork, not an image of text, so 1.4.5 is still satisfied — but the premise is now false, and **SC 1.1.1 Non-text Content becomes live for the first time in this product**. Zerado's answer is already built and should be recorded as the brief's answer: **the caption is the text alternative, it is always present, it is never optional, and it is what remains when the picture is not there** (§8.1, §13). The line needs rewriting rather than deleting | `00-design-brief.md` §3 | `fft-design-architect` |
